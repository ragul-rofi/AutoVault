...
def create_physical_file(machine_id, filename, content):
...
%"""
    ...
    path_ph_v1 = create_physical_file(101, "pump_housing_v1.nc", ph_v1_content)
    ...
    cur.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("pump_housing.nc", 101, user_ids["admin"], 1, hash_ph_v1, path_ph_v1))

    path_ph_v2 = create_physical_file(101, "pump_housing_v2.nc", ph_v2_content)
    hash_ph_v2 = calculate_hash(ph_v2_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("pump_housing.nc", 101, user_ids["admin"], 2, hash_ph_v2, path_ph_v2))
...
%"""
    ...
    path_tb_v1 = create_physical_file(102, "turbine_blade_v1.nc", tb_v1_content)
    hash_tb_v1 = calculate_hash(tb_v1_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("turbine_blade.nc", 102, user_ids["engineer"], 1, hash_tb_v1, path_tb_v1))

    path_tb_v2 = create_physical_file(102, "turbine_blade_v2.nc", tb_v2_content)
    hash_tb_v2 = calculate_hash(tb_v2_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("turbine_blade.nc", 102, user_ids["engineer"], 2, hash_tb_v2, path_tb_v2))

    path_tb_v3 = create_physical_file(102, "turbine_blade_v3.nc", tb_v3_content)
    hash_tb_v3 = calculate_hash(tb_v3_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("turbine_blade.nc", 102, user_ids["engineer"], 3, hash_tb_v3, path_tb_v3))
...
M30"""
    ...
    path_vs_v1 = create_physical_file(103, "valve_seal_v1.gcode", vs_v1_content)
    hash_vs_v1 = calculate_hash(vs_v1_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("valve_seal.gcode", 103, user_ids["admin"], 1, hash_vs_v1, path_vs_v1))

    path_vs_v2 = create_physical_file(103, "valve_seal_v2.gcode", vs_v2_content)
    hash_vs_v2 = calculate_hash(vs_v2_content.encode('utf-8'))
    cur.execute("""
        INSERT INTO file_versions (file_name, machine_id, uploaded_by, version_no, file_hash, storage_path)
        VALUES (%s, %s, %s, %s, %s, %s);
    """, ("valve_seal.gcode", 103, user_ids["admin"], 2, hash_vs_v2, path_vs_v2))
    ...
    audit_logs_data = [
        (user_ids["viewer"], 101, "DOWNLOAD", "bracket_arm.cnc", 4, "2026-05-24 09:45:00"),
        (user_ids["admin"], 101, "DIFF", "valve_seal.gcode", 2, "2026-05-24 11:04:15"),
        (user_ids["engineer"], 102, "UPLOAD", "turbine_blade.nc", 3, "2026-05-24 13:15:42"),
        (user_ids["admin"], 101, "ROLLBACK", "pump_housing.nc", 2, "2026-05-24 14:32:10")
    ]
